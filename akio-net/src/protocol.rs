...
fn bind_client<C: ClientState, T: AsyncRead + AsyncWrite>(io: T)
    -> (
...
fn serialize_message<T>(message: T) -> Result<BytesMut>
    where T: protobuf::MessageStatic
{
    message.write_to_bytes().map(|x| x.into()).map_err(|e| e.into())
}
...
fn do_handshake<S, R>(
    ...
    where S: Sink<SinkItem = rpc::Request, SinkError = ()>,
          R: Stream<Item = rpc::Response, Error = Error>
{
    ...
    sender.send(request).and_then(|sender2| {
        ...
        response.ok_or(()).and_then(|inner| {
            if inner.error == rpc::ErrorCode::NONE && inner.has_finish_handshake() {
                let uuid = Uuid::from_bytes(&inner.get_finish_handshake().client_id).map_err(|_| ())?;
...
fn receive_handshake<S, R>(
    ...
    where S: Sink<SinkItem = rpc::Response, SinkError = ()>,
          R: Stream<Item = rpc::Request, Error = Error>
{
    receiver.into_future()
        ...
        .and_then(move |(mut request, receiver)| {
            ...
            sender.send(response).map(move |sender| (request.take_start_handshake(), sender, receiver))
        })
}
...
fn when_connected<T, S, R>(
    ...
    where T: ClientState + Clone + 'static,
          ...
          R: Stream<Item = T::Receive, Error = Error> + 'static
{
    ...
    let connection = send_stream.map(|_| ()).select(socket_read_stream.map(|_| ())).then(move |_| {
...
) -> impl Future<Item = ClientRxState, Error = ()>
{
    ...
    receive_handshake(my_id, sender, receiver)
        .map(move |(handshake, sender, receiver)| {
            when_connected(
                handshake,
                sender,
                receiver,
...
) -> impl Future<Item = ClientTxState, Error = ()>
{
    ...
    do_handshake(my_id, &listen_addr, sender, receiver)
        .map(move |(client_id, sender2, receiver2)| {
            when_connected(
                client_id,
                sender2,
                receiver2,
