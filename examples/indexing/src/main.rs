...
impl OrganizationActor {
    ...
    pub fn debug(&mut self) {
        println!("id: {}, name: {}, owner: {}", self.id(), self.name, self.owner)
    }
}
...
pub fn main() {
    ...
    system.on_startup(|| {
        ...
        orgs.add(Uuid::new_v4(), "org 1".to_string(), "kphelps@salsify.com".to_string());
        orgs.add(Uuid::new_v4(), "org 2".to_string(), "kylep91@gmail.com".to_string());
        orgs.add(Uuid::new_v4(), "org 3".to_string(), "meh@meh.meh".to_string());
        orgs.count();
    });
    system.start();
}
