...
pub fn with_actor_system_async<F, R>(f: F) -> R
    where F: FnOnce(ActorSystem) -> Box<Future<Item = R, Error = ()> + Send + 'static> + Send + 'static,
          R: Send + 'static
{
    ...
    system.on_startup(move || {
                          let fut = f(system_clone.clone())
                              .map(|f_result| sender.send(f_result).ok().unwrap());
                          context::execute(fut);
                      });
...
pub fn with_actor_system<F, R>(f: F) -> R
    where F: FnOnce(ActorSystem) -> R + Send + 'static,
          R: Send + 'static
{
